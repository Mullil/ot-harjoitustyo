...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_syo_edullisesti_ei_vie_saldoa_negatiiviseksi(self):
        for _ in range(5):
            self.kortti.syo_edullisesti()
        self.assertFalse(self.kortti.saldo_euroina() < 0)
    ...
    def test_edullisen_lounaan_voi_ostaa_kun_on_rahaa_edullisen_lounaan_verran(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(kortti.saldo_euroina(), 0.0)

    def test_maukkaan_lounaan_voi_ostaa_kun_on_rahaa_maukkaan_lounaan_verran(self):
